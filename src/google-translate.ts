...
import type {
    TranslateOptions, GoogleError, GoogleToken, Translation 
} from './types'
...
export class Translator {
    constructor(googleApp: GoogleApp) {
        this.gApp = googleApp;
    }

    gApp: GoogleApp;

    async translate(text: string | string[], opts: TranslateOptions): Promise<Translation[]> {
        opts = opts || {};
        ...
        try {
            return await new Promise<Translation[]>(function (resolve, reject) {
                ...
                req.on('error', (err: GoogleError) => { Translator.#errHandler(err, reject); });
    ...
    #refreshToken(): Promise<GoogleToken> {
    ...
    static #errHandler(err: GoogleError, reject: any) {
        if (err?.errno == -54) {
