...
import type {
    Translations, AppDetails, AppParameter, AppTranslation, AppManifest
} from "./types";
...
const MAX_LENGTHS: { [key: string]: number } = {
    name: 40,
...
var gtranslate:googletranslate.Translator;
(async function() {
...
function getReadmeSection(readme:string, key:string):string {
    let newDetail = '';
...
function updateAppDetailsFromReadme(translations: Translations) {
    ...
    let updated = false;
    let appObj: AppDetails = {...translations[DEFAULT_LOCALE]["app"]};
...
function updateManifestFromReadme() {
    const readme = fs.readFileSync(README).toString();
    let manifest: AppManifest = JSON.parse(fs.readFileSync(MANIFEST).toString());
...
function validateManifest(translations:Translations) {
    ...
    for (let key in params) {
        let manip = mani["parameters"].filter((p:AppParameter) => { return p["name"] == key; });
        if (manip.length != 1) {
...
async function updateTranslations(translations:Translations) {
...
function checkForUpdates(translations: Translations): boolean {
...
function writeTranslationsToFile(translations: Translations) {
    const translationsString = JSON.stringify(translations);
    const translationsObj = `export const translations = ${translationsString};`;
...
function argOrEnv(argNum: number, envKey: string): string {
